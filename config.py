class Agent:
    batch_size = 32 # Number of samples to use for Q Learning
    exploration_rate = 0.6 # Percent of time to use random choice (decimal)
    ...
    action_probability = [0.2, 0.2, 0.1, 0.05, 0.05, 0.2, 0.2]
...
class environment:
    ...
    skip_frame_num = 4 # Number of frames to skip for each cycle
    stack_frame_num = 4 # Number of frames to stack as series data for model
    ...
    actions = [["right"], ["right", "A"], ["A"], ["left"], ["left", "A"], ["B", "right"], ["B", "right", "A"]]
